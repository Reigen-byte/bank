 <?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['isimti'])) {
        foreach ($accounts as $i => $account) {
            if ($account['IBAN'] == $id) {
                if ((int) $_POST['isimti'] <= $account['accountMoney']) {
                    $account['accountMoney'] -= (int) $_POST['isimti'];
                    $accounts[$i] = $account;
                    break;
                } else {
                    $_SESSION['error'] = 'Negalite nuskaičiuoti daugiau lėšų nei yra sąskaitoje';
                    header('Location: http://localhost/phpnd/bank/accounts.php');
                    exit;
                }
            }
        ...
        $_SESSION['success'] = "Lėšos sėkmingai nuskaičiuotos";
                         ...
                         <div class="card w-50 mt-5">
                             <div class="card-body">
                                 ...
                                 <form action="" method="post">
                                     <input type="text" name="isimti">
                                     <button type="submit" class="btn btn-primary">Nuskaičiuoti</button>
                                 </form>
                             </div>
                         </div>
                     </div>
